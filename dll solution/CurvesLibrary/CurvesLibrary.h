...
extern "C" CURVESLIBRARY_API class Curve3D {
public:
...
// Circle class
extern "C" CURVESLIBRARY_API class Circle : public Curve3D {
private:
    double radius;
public:
    Circle(double r) : radius(max(r, 0.0)) {}
...
// Ellipse class
extern "C" CURVESLIBRARY_API class Ellipse : public Curve3D {
private:
    double a, b; // semi-major and semi-minor axes
public:
    Ellipse(double major, double minor) : a(max(major, 0.0)), b(max(minor, 0.0)) {}

    double getRadius() const override { return max(a, b); }
...
// 3D Helix class
extern "C" CURVESLIBRARY_API class Helix : public Curve3D {
private:
    double radius; // Radius of the helix
public:
    Helix(double r) : radius(max(r, 0.0)) {}

    double getRadius() const override { return radius; }
    double getX(double t) const override { return radius * std::cos(t); }
    double getY(double t) const override { return radius * std::sin(t); }
    double getZ(double t) const override { return t; }
    double getDerivativeX(double t) const override { return -radius * std::sin(t); }
    double getDerivativeY(double t) const override { return radius * std::cos(t); }
    double getDerivativeZ(double t) const override { return 1; }
...
extern "C" CURVESLIBRARY_API double parallelSum(const std::vector<Circle*> vec, int start, int end);